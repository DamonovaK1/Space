...
const shaderId = 'Purple Star Trails'
...
function init() {
  // Load shader source from script tag or external file
  source = document.querySelector("script[type='x-shader/x-fragment']")
  if (!source || !source.textContent) {
    // If shader is in external file, load it
    loadShaderFromFile()
    return
  }
  ...
  // Initialize hovering text animation
  initHoveringText()
...
// Load shader from external file or GitHub repository
async function loadShaderFromFile() {
  // GitHub repository raw file URL
  const githubRawUrl = 'https://raw.githubusercontent.com/DamonovaK1/Space/main/shader.glsl'
  
  try {
    // Try to load from GitHub repository first
    console.log('Attempting to load shader from GitHub repository...')
    ...
    // Validate that we got actual shader code
    if (!shaderCode.includes('#version') && !shaderCode.includes('void main')) {
      throw new Error('Invalid shader code received from GitHub')
    }
    
    console.log('Successfully loaded shader from GitHub repository!')
    console.log('Shader preview:', shaderCode.substring(0, 100) + '...')
    
    // Create a script element with the shader code
    ...
    console.warn('Failed to load from GitHub:', error.message)
    console.log('Trying local shader file...')
    ...
    try {
      ...
      // Continue initialization
      source = script
      console.log('Successfully loaded local shader file!')
      init()
      
    } catch (localError) {
      console.error('Failed to load local shader file:', localError)
      console.log('Using embedded fallback shader...')
      
      // Final fallback to embedded shader with purple stars effect
      const defaultShader = `#version 300 es

/*********
* Purple Stars with Trails - Neon Blue Galaxy
* Fallback shader - embedded version
*/
...
void main(void) {
	...
	O=vec4(col,1);
      ...
      const script = document.createElement('script')
      script.type = 'x-shader/x-fragment'
      script.textContent = defaultShader
      document.head.appendChild(script)
      source = script
      init()
    }
...
// Hovering text animation functions
function initHoveringText() {
  const hoveringText = document.getElementById('hovering-text')
  
  // Track mouse movement for text positioning
  document.addEventListener('mousemove', (e) => {
    ...
    hoveringText.style.transform = `translate(${-50 + offsetX}%, ${-50 + offsetY}%)`
  })
  
  // Start fracture animation after 5 seconds
  setTimeout(() => {
    fractureText(hoveringText)
  }, 5000)
}

function fractureText(element) {
  ...
  // Create fracture pieces for each character
  ...
  characters.forEach((char, index) => {
    if (char === ' ') {
      ...
      space.innerHTML = '&nbsp;'
      space.className = 'fracture-piece'
      span.appendChild(space)
      pieces.push(space)
    } else {
      const piece = document.createElement('span')
      piece.textContent = char
      piece.className = 'fracture-piece'
  ...
  pieces.forEach((piece, index) => {
    ...
    const randomRotStart = (Math.random() - 0.5) * 180 + 'deg'
    const randomRotEnd = (Math.random() - 0.5) * 720 + 360 + 'deg'
    const delay = index * 50 + Math.random() * 200
    
    piece.style.setProperty('--random-x', randomX)
    piece.style.setProperty('--random-rot', randomRotStart)
    piece.style.setProperty('--random-rot-end', randomRotEnd)
    
    setTimeout(() => {
      piece.style.animation = `float-up ${2 + Math.random()}s ease-out forwards`
    }, delay)
  })
  
  // Remove the entire element after animation
  setTimeout(() => {
    element.remove()
  }, 4000)
}
